  ...
  return (
    <nav className={`fixed top-0 left-0 w-full py-4 px-6 flex items-center justify-between transition-all duration-300 z-50 ${
      ...
      <div className="flex justify-center mx-4">
        <div className="bg-violet-950 bg-opacity-60 rounded-full border-2 border-purple-900 drop-shadow-blue-900-xl px-4 py-2 flex gap-6 shadow-neumorphic-inset">
          {['Home', 'About', 'Contact'].map((item) => (
            <div 
              key={item} 
              className="relative group"
            >
              <button className="text-[#a277ff] px-3 py-1 rounded-lg group-hover:text-[#bd93f9] group-hover:scale-110 transition-all duration-300">
                {item}
              </button>
