  ...
  return (
    <nav className={`fixed top-0 left-0 w-full py-4 px-6 flex items-center justify-between transition-all duration-300 z-50 ${
      isScrolled 
        ...
        : 'bg-gradient-to-r from-neugray-900 to-neopurple-900 shadow-neumorphic'
    }`}>
      {/* Logo */}
      ...
      <div className="flex justify-center mx-4">
        <div className="bg-neugray-800 bg-opacity-50 rounded-full px-4 py-2 flex gap-6 shadow-neumorphic-inset">
          {['Home', 'About', 'Contact'].map((item) => (
            <div 
              key={item} 
              className="relative group"
            >
              <button className="text-gray-200 px-3 py-1 rounded-lg group-hover:text-neopurple-300 group-hover:scale-110 transition-all duration-300">
                {item}
              </button>
      ...
      <div className="flex items-center gap-2">
        {isLoggedIn ? (
          <Button 
            variant="ghost" 
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-neugray-800 bg-opacity-50 border border-neopurple-500/30 shadow-neumorphic-sm text-neopurple-300"
          >
            <User size={18} />
            <span>User</span>
          </Button>
          ...
          <Button 
            onClick={handleLogin}
            variant="outline"
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-neugray-800 bg-opacity-50 border border-neopurple-500/30 shadow-neumorphic-sm hover:shadow-neon hover:border-neopurple-400 transition-all duration-300 text-gray-200 hover:text-neopurple-300 group"
          >
