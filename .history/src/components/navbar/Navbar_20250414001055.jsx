...
export default function Navbar() {
    ...
    {['Home', 'About', 'Contact'].map((item) => (
      ...
      <div className="flex items-center gap-2">
        {isLoggedIn ? (
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-black bg-opacity-70 border !border-violet-900 !text-violet-300 shadow-neumorphic-sm hover:border-violet-400 hover:text-violet-200 transition-all duration-300"
        >
            <User size={18} />
            <span>User</span>
          </Button>
  ...
  onClick={handleLogin}
  variant="outline"
  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-black bg-opacity-70 !border-violet-500 !text-violet-300 shadow-neumorphic-sm hover:!border-violet-400 hover:!text-violet-200 transition-all duration-300"
>
  <LogIn size={1} className="group-hover:rotate-12 transition-all duration-300" />
  <span>Sign In</span>
</Button>
