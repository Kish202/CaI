    ...
    <nav className={`fixed top-0 left-0 w-full py-4 px-6 flex items-center justify-between transition-all duration-300 z-50 ${
      ...
      <div className="flex items-center">
        <div className="group transition-all duration-300 cursor-pointer">
          ...
          <span className="ml-2 text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-300 to-purple-500 group-hover:from-purple-200 group-hover:to-purple-400">CaI</span>
        </div>
      </div>
  ...
  <div className="bg-violet/60 rounded-full px-4 py-2 flex gap-6 shadow-inner shadow-purple-700/90 hover:shadow-purple-500/70 transition-all duration-300">
    {['Home', 'About', 'Contact'].map((item) => (
      <div key={item} className="inline-flex">
        <button className="text-slate-300/70 px-3 py-1 text-[20px] rounded-2xl border border-transparent transition-all duration-300 hover:scale-110 hover:text-slate-300/90 hover:border-violet-700/70 transform origin-center">
          {item}
        </button>
      </div>
    ))}
...
<Button 
  className={`flex items-center gap-2 px-4 py-2 rounded-xl 
             ...
             hover:border-violet-700/30 hover:text-violet-200 
             hover:bg-transparent
             ${isScrolled ? 'bg-purple-700/90 ' : 'bg-transparent'}
    ...
    <Button 
      ...
      className={`flex items-center gap-2 px-4 py-2 rounded-xl 
        ...
        hover:border-violet-700/30 hover:text-violet-200 
        hover:bg-transparent
        ${isScrolled ? 'bg-purple-700/30 ' : 'bg-transparent'}
        transition-all duration-300`}
>
